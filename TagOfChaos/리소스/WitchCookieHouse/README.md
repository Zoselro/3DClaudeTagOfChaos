...
| 문짝 | 1.44 x 2.97 x 0.15 m | 틈 3 cm |
...
| `Animations/Door_{Side}_Open/Close.anim` | 문 클립 8개 (30fps, 18프레임 = 0.6초) |
| `Animations/Door_{Side}.controller` | 문별 Animator Controller (Bool `IsOpen`) |
...
- 4개 문 모두 **집 안쪽으로 열림**, 경첩은 바깥에서 봤을 때 왼쪽.
- Unity 기준 닫힘 `localEulerAngles = (0, 0, 0)`, 열림 `localEulerAngles.y = -90` (4개 문 동일).
- 열린 문짝은 안쪽 벽에 붙어 개구부(폭 1.5 m)를 가리지 않는다.
- Blender에서 0~90° 구간을 3° 간격으로 검사: 문짝이 문틀·벽·장식·내부 소품과 겹치지 않음. 충돌체끼리도 겹치지 않음.
- 제어: 문마다 Animator가 따로 있으므로 개별 제어 가능.

```csharp
door.GetComponent<Animator>().SetBool("IsOpen", true);   // Door_{Side}_Open
door.GetComponent<Animator>().SetBool("IsOpen", false);  // Door_{Side}_Close
```

  멀티플레이에서는 이 Bool을 RPC나 PhotonAnimatorView로 동기화해야 한다(현재 동기화 코드는 없음).
