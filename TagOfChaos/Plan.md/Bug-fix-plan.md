...
> 해결 방안까지 수립했으나, **근본 수정(A/B안) 자체는 아직 적용하지 않았다**(§19). 다만 그 사이
> 별도로 구현된 `PlayerControllPlan.md` §24(점프 중 공중 자유 방향전환)의 부가 효과로 좁은
> 틈을 미리 피할 수 있게 되면서, **사용자가 실제 플레이에서 이 Stuck 현상이 더 이상 재현되지
> 않음을 확인했다(§19.9, 2026-08-16)** — 근본 원인(non-convex 콜라이더)은 그대로 남아있으므로
> 완전 해결로 보지는 않으며, 재발 방지용 A/B안 적용 여부는 사용자 판단에 맡긴다.
>
> **⑪(신규, 2026-08-17 추가, 같은 날 구현·검증까지 완료)** — `PlayerControllPlan.md` §25(Cookie
> 애니메이션 세트 마이그레이션) 구현 후 사용자 플레이 테스트에서 "색칠할 때 하체는 칠해지지만
> 상체는 칠해지지 않는다"와 "붓 커서가 캐릭터 몸속으로 파고든다" 두 가지 버그가 함께 보고됐다.
> Unity MCP로 직접 실측 조사한 결과 **두 증상의 원인이 동일**함을 확정했다 — `Mesh_0`의
> `MeshCollider`가 스킨 애니메이션을 따라가지 않고 임포트 시점의 바인드 포즈(정지 자세)에 고정돼
> 있는데, 새 Cookie 캐릭터의 바인드 포즈가 옛 `Ch36`(거의 정확히 서 있는 자세)과 달리 실제 서
> 있는 Idle 자세와 전혀 다른 형태라서, 화면상 상체 위치로 레이캐스트를 쏘면 얼어붙은 콜라이더가
> 그 자리에 없어 아예 빗나가고(색칠 실패), 하체 근처는 우연히 맞아도 그 지점이 실제 눈에 보이는
> 표면과 다른 위치라 커서가 몸 안쪽에 박힌 것처럼 보였다. 로컬 플레이어의 `Mesh_0`를 실시간으로
> 다시 구워 콜라이더에 반영하는 방식으로 구현했고, 그 과정에서 캐릭터 자체 스케일(100배)이
> 이중 적용되는 버그와 매 프레임 갱신 시 프레임레이트가 257fps→15fps로 급락하는 성능 문제를
> 추가로 발견해 스케일 역보정 + 3프레임 스로틀링(→47fps 개선)으로 함께 해결했다. 원인 조사부터
> 구현·실측 검증까지 전부 §20에 정리했다.
>
> **⑫(신규, 2026-08-17 추가)** — 사용자가 "Dodge 할 때 특정 프레임들에서 공중에 떠있는 현상이
> 있다"고 보고했다. `Time.timeScale=0`으로 고정하고 회피 동작을 프레임 단위로 재생하며 발(뼈대)
> 좌표를 직접 추적한 결과, 캐릭터의 물리 루트는 완전히 지면에 고정돼 있는데(`rb.position.y`
> 항상 `0`) **발이 최대 1.28m까지 떠오르는 것을 실측으로 확인**했다. 원인은 `Cookie_Dodge`
> 모션캡처 소스가 실제로는 제자리 회피가 아니라 몸 전체가 앞으로 약 4m 이동하며 회전하는 진짜
> "다이빙 롤"이고(휴머노이드 루트 모션 커브 `RootT`/`RootQ`를 직접 조회해 확인), 프로젝트가
> `applyRootMotion=false`로 이동을 전부 코드가 수동 처리하는 구조라 이 루트 모션이 실제 캐릭터
> 위치에는 반영되지 않고 메시(스켈레톤)에만 남아 허공에서 뒤집히는 것처럼 보이는 것이다. 이후
> 사용자가 "백업본을 만들어놓고 B안으로 수정후 구현해달라"고 지시해, 수정 전 클립을 백업한 뒤
> 루트 모션 7개 커브를 0번 프레임 값으로 고정하는 방식으로 구현·검증까지 완료했었다(발의 최고
> 높이가 1.28m에서 0.375m로 개선). **하지만 사용자가 실제 결과를 확인한 뒤 "다시 되돌려줘 다른
> 방안이 필요한거같아"라고 요청해, 백업 파일로 완전히 원상복구했다 — 이 버그는 현재
> 미해결 상태이며, 다른 수정 방향을 다시 검토해야 한다.** 상세는 §21.
...
| ⑩ | 오브젝트 사이(테이블+의자+파라솔)로 점프하면 캐릭터가 완전히 고착돼 이동·구르기가 전혀 안 됨 | 🟢 **원인 실측 확정, 근본 수정(A/B안)은 미구현이지만 §24(공중 자유 방향전환)의 부가 효과로 실사용에서 재현 안 됨을 사용자가 확인** | 사용자가 겪은 실제 고착 인스턴스를 Unity 라이브 세션에서 직접 조사 — 캐릭터 캡슐이 `Parasol_Canopy`/`Table_Top`/`ChairA_Seat` 3개의 non-convex `MeshCollider`와 동시에 겹쳐 있었고, 실제 게임 조건에서 강제 속도를 줘도 전혀 움직이지 않았다. 1차 원인은 좁은 틈에서 여러 non-convex 메시 콜라이더가 모순된 접촉을 만드는 것(씬 전체 26개 콜라이더가 전부 non-convex), 2차 원인은 `Move()`가 매 프레임 물리 보정을 무조건 덮어써 스스로 빠져나올 수단이 없는 것. 근본 수정(소품 `MeshCollider`를 convex로 전환)은 아직 미적용 상태지만, `PlayerControllPlan.md` §24(점프 중 공중 자유 방향전환)를 구현한 이후로는 좁은 틈을 피할 여지가 생겨 실제 플레이에서 더 이상 재현되지 않음을 사용자가 확인했다(§19.9) — `Crate`/`Fence` 군집은 여전히 구조적으로 동일 위험 보유 |
...
위험). 해결 방안(A/B안)까지 수립했으나, **코드/에셋 수정(A/B안)은 아직 적용하지 않았다.**

### 19.9 [추가, 2026-08-16] 실사용에서 증상 재현 안 됨 — `PlayerControllPlan.md` §24(공중 자유
방향전환)의 부가 효과로 확인

이 절을 조사·논의하던 중 나온 `PlayerControllPlan.md` §24("의도한 점프도 낙하와 동일하게 공중
에서 자유로운 방향전환 허용")가 그 사이 별도로 구현·배포됐고, 사용자가 실제 플레이로
검증한 결과 **§24 적용 이후로는 오브젝트 사이 점프 시 Stuck 현상이 더 이상 재현되지 않음을
확인했다.** §24.1에서 이미 예상했던 부가 효과("좁은 틈을 향해 날아가고 있다는 걸 눈치챘을 때
공중에서 스스로 피할 여지가 생겨 Stuck류 상황에 빠질 확률 자체가 줄어든다")가 실제로 체감될
만큼 작동한 것으로 보인다.

**단, 이건 §19.3의 근본 원인(non-convex 소품 콜라이더가 좁은 틈에서 모순된 접촉을 만드는 것)이
고쳐졌다는 뜻은 아니다** — `Table/Chair/Parasol`을 포함한 씬의 26개 non-convex `MeshCollider`는
§19.6의 A/B안(convex 전환)이 여전히 적용되지 않은 그대로다. §24는 플레이어가 "그 틈으로
들어가는 것 자체를 피하게" 도와줄 뿐, 만에 하나 다시 그 틈에 들어가면(예: 방향전환이 늦었거나,
좁은 틈 자체를 인지하지 못했거나, 넉백/충돌 등으로 떠밀린 경우) §19.2에서 실측한 완전 고착
메커니즘 자체는 여전히 그대로 남아있다.

**정리**: 실사용 기준으로는 증상이 사라져 **체감 우선순위는 낮아졌지만**, 근본 원인은 미수정
상태로 남아있다 — §19.6의 A/B안(소품 `MeshCollider`를 convex로 전환 + 파라솔 지붕 충돌 제거)을
아예 안 할지, 아니면 재발 방지용 방어 조치로 나중에 한 번 더 적용할지는 사용자 판단이 필요하다.
`Crate`/`Fence` 군집(§19.4)도 여전히 구조적으로 동일한 위험을 안고 있다는 점은 참고할 것.

---

## 20. ⑪ Cookie 마이그레이션 후 색칠 상체 누락 + 붓 커서가 몸속으로 파고드는 버그 — ✅ 원인 확정·구현·실측 검증 완료 (2026-08-17)

> 사용자 보고: "색칠할 때 하체는 색이 칠해지지만, 상체는 색이 칠해지지 않는 버그가 있음" +
> "붓이 플레이어 몸에 들어가는 현상이 있음. 붓이 몸에 들어가면 안 되고 사용자 입장에서 육안으로
> 붓이 보여야 한다." `PlayerControllPlan.md` §25(Cookie 애니메이션 마이그레이션) 구현 직후 실제
> 플레이 테스트에서 발견된 회귀다. Unity MCP `execute_code`로 Play Mode에서 직접 레이캐스트를
> 쏘고 콜라이더/렌더러의 실제 월드 바운드를 비교해 원인을 확정했다. **지시에 따라 이번에는 계획만
> 정리했고 실제 수정은 진행하지 않았다.**

### 20.1 증상

- 색상 라운드 중 캐릭터 하반신(다리)을 클릭하면 정상적으로 칠해지지만, 상반신(가슴·팔·머리
  근처)을 클릭하면 전혀 칠해지지 않는다.
- 3D 붓 커서(`BrushCursorController`)가 마우스 위치를 따라다니긴 하지만, 캐릭터 몸 표면 위에
  뜨는 게 아니라 몸 안쪽으로 파고든 것처럼 보인다.

### 20.2 원인 조사 — 실측

`PlayerPaintCanvas`/`BrushCursorController` 둘 다 캐릭터 몸(`Mesh_0`)의 `MeshCollider`를
`Physics.Raycast`의 대상(`paintableCollider`)으로 쓴다(§1.6). Play Mode에서 실제 살아있는
캐릭터 인스턴스를 대상으로 `SkinnedMeshRenderer.bounds`(현재 화면에 실제로 렌더링되는, Idle
애니메이션이 적용된 월드 바운드)와 `MeshCollider.bounds`(물리 충돌 판정에 쓰이는 바운드)를
나란히 조회했다:

| | `SkinnedMeshRenderer.bounds`(실제 화면에 보이는 위치) | `MeshCollider.bounds`(레이캐스트가 실제로 맞는 위치) |
|---|---|---|
| 중심(center) | `(0.02, 0.93, 0.00)` | `(0.00, 0.00, 0.04)` |
| 크기(size) | `(1.19, 2.11, 0.77)` | `(1.88, 1.97, 0.36)` |

**중심 Y좌표가 `0.93`(가슴 높이) vs `0`(거의 발밑) — 완전히 다른 위치를 가리킨다.** 실제로 화면에
보이는 캐릭터의 가슴·머리 위치로 레이캐스트를 쐈을 때 콜라이더가 그 자리에 없어서 아예 빗나가고,
반대로 콜라이더가 실제로 존재하는(하지만 화면상 캐릭터 위치와는 다른) Y 구간으로 쏘면 맞는지를
직접 대조했다:

| 조준 대상(화면에 보이는 실제 위치 기준) | 결과 |
|---|---|
| 가슴 (world Y=1.43) | **빗나감(hit=False)** |
| 머리 (world Y=1.83) | **빗나감(hit=False)** |
| 몸통 중심 (world Y=0.93) | 명중 (`Mesh_0`, uv=(0.82, 0.01)) |
| 다리 (world Y=0.23) | 명중 (`Mesh_0`, uv=(0.73, 0.20)) |
| 발 (world Y=-0.07) | 명중 (`Mesh_0`, uv=(0.69, 0.27)) |

**"하체는 칠해지고 상체는 안 칠해진다"는 사용자 증상과 정확히 일치한다.**

### 20.3 근본 원인 — `MeshCollider`는 스킨 애니메이션을 따라가지 않는다

`Mesh_0`는 `SkinnedMeshRenderer`로 렌더링되는 스킨 메쉬다. 화면에 실제로 그려지는 형태(포즈)는
매 프레임 뼈대(`Armature`/`mixamorig:*`)의 애니메이션에 따라 GPU에서 실시간으로 변형되지만,
`MeshCollider.sharedMesh`(물리 충돌·레이캐스트가 참조하는 실제 지오메트리)는 **에셋을 처음
읽어들일 때의 정지 자세(바인드 포즈) 그대로 고정돼 있고, 애니메이션에 전혀 반응하지 않는다** —
이것은 이 프로젝트만의 버그가 아니라 Unity의 잘 알려진 구조적 제약이다(`MeshCollider`는 스킨
변형을 인식하지 못함).

`Mesh_0`의 바인드 포즈 자체를 직접 조회한 결과, 로컬 정점 Y좌표가 대략 `[-0.98, 0.98]` 범위에
집중돼 있다 — **서 있는 자세가 아니라 캐릭터의 중심에 가깝게 압축된, 서 있는 Idle 포즈와는
완전히 다른 참조 자세**다. `MeshCollider`는 이 자세 그대로 얼어붙어 있으므로, 실제로 서서
움직이는 캐릭터의 화면상 위치와 큰 폭으로 어긋난다.

### 20.4 왜 옛 `Ch36`에서는 이 문제가 안 보였는가 (재확인)

옛 `Ch36`(`Assets/Animation/Old/Idle.fbx`)의 바인드 포즈를 동일한 방식으로 조회한 결과, 로컬 Y
범위가 `[0.004, 1.769]`였다 — **발(≈0)부터 머리(≈1.77)까지, 실제로 서 있는 자세와 이미 거의
일치**한다. 즉 옛 모델은 우연히(혹은 의도적으로) 바인드 포즈 자체가 Idle 서 있는 자세와 비슷해서
콜라이더-비주얼 불일치가 육안으로 드러나지 않을 만큼 작았을 뿐, **같은 구조적 결함을 처음부터
안고 있었다.** Cookie 캐릭터는 바인드 포즈가 서 있는 자세와 크게 달라서(§20.3) 같은 결함이
뚜렷하게 드러난 것이다 — 새로 생긴 버그가 아니라 원래 있던 취약점이 이번에 노출된 것.

### 20.5 두 번째 증상(붓 커서가 몸속으로 파고듦)도 같은 원인

`BrushCursorController.Update()`(§1.6)는 레이캐스트가 명중했을 때 `hit.point`/`hit.normal`을
그대로 커서 위치/방향으로 쓴다:
```csharp
cursorInstance.transform.SetPositionAndRotation(hit.point, Quaternion.FromToRotation(Vector3.up, hit.normal));
```
콜라이더가 얼어붙은 바인드 포즈 표면에 있으므로, `hit.point`는 **화면에 실제로 보이는(애니메이션이
적용된) 표면 위치가 아니라 그와 다른, 종종 그 안쪽에 위치한 지점**이다. 사용자가 "붓이 몸속으로
들어간다"고 느끼는 것은 정확히 이 좌표 불일치의 시각적 결과다 — 레이캐스트 자체는 콜라이더 표면에
정확히 닿았지만, 그 콜라이더가 화면에 보이는 실제 표면과 다른 곳에 있어서 커서가 "보이는 몸통의
안쪽"에 놓이는 것처럼 보인다. 추가로, 코드 자체에 표면에서 살짝 띄우는 법선 방향 오프셋이 전혀
없다는 점(`hit.point`를 그대로 사용, 여유값 없음)도 부차적으로 겹친다 — 콜라이더 위치가 정확히
맞더라도 모델의 두께에 따라 살짝 파고들어 보일 여지가 있다(§20.7).
...
| 방안 | 설명 | 채택 여부 |
|---|---|---|
| A. 로컬 플레이어의 `Mesh_0`를 매 프레임(또는 라운드 진행 중) `SkinnedMeshRenderer.BakeMesh()`로 구워 `MeshCollider.sharedMesh`를 실시간 갱신 | 콜라이더가 항상 현재 애니메이션 포즈를 그대로 반영 — UV/토폴로지가 렌더 메쉬와 100% 동일하므로 `hit.textureCoord` 정확도도 그대로 유지됨 | **채택** |
| B. 바인드 포즈를 Idle 자세에 맞춰 다시 구워서 정적으로 고정 | Idle 상태에서는 맞아떨어지지만 Walk/Run/Jump/Dodge 등 다른 포즈에서는 여전히 어긋남 — 근본 해결 아님 | 기각 |
| C. 뼈대에 붙는 여러 개의 프리미티브 콜리더(캡슐 등)로 몸 부위별 근사 콜라이더 구성 | 뼈를 따라 정확히 움직여서 위치 문제는 해결되지만, `hit.textureCoord`가 나오지 않아 UV 기반 정밀 페인팅 자체가 불가능해짐 — 현재 붓칠 시스템(UV 스탬프)의 전제와 정면으로 충돌 | 기각 |

**A안 세부 설계 (구현 완료)**:
1. ~~`PlayerPaintCanvas`에 재사용 가능한 `Mesh` 필드 추가~~ → `bakedColliderMesh` 필드로 구현,
   `Start()`에서 1회 생성.
2. ~~로컬 플레이어 전용~~ → `Update()`가 `pv.IsMine` 가드를 이미 통과한 뒤에만 굽기를 수행하도록
   구현(기존 구조 그대로 재사용).
3. ~~`Update()` 최상단, 레이캐스트 전에 갱신~~ → 구현 완료, 위치도 계획대로.
4. ~~성능 고려/완화~~ → **구현 중 실제로 심각한 성능 문제를 실측으로 확인하고 스로틀링을
   추가했다.** 자세한 내용은 §20.6-7 참고.
5. ~~`sharedMesh` 재할당 방식 확인~~ → **실측 확정: 같은 `Mesh` 객체 참조를 내용만 바꿔 재대입하면
   PhysX가 전혀 갱신을 인식하지 못한다**(재대입 비용이 `0.0001ms`로 사실상 no-op이었음, §20.6-7의
   프로파일링 결과). `sharedMesh = null` 후 다시 대입해야만 실제로 재계산(cook)됨을 확정하고 그
   방식으로 구현했다.
6. ~~`BrushCursorController`는 별도 수정 불필요~~ → 맞았다. `PaintableCollider` 참조를 그대로
   쓰므로 A안 적용만으로 자동으로 정확해졌다(법선 오프셋만 별도로 추가, §20.7).

### 20.6-7 [구현 중 신규 발견 ①] `BakeMesh()` 결과에 캐릭터 자체 스케일(100배)이 이중 적용되는 버그

계획대로 `skinnedBodyRenderer.BakeMesh(bakedColliderMesh)` 후 그대로 `MeshCollider.sharedMesh`에
대입했더니, 콜라이더가 실제 몸보다 **약 100배** 커지는 새로운 문제가 발생했다(`MeshCollider.bounds`
중심이 `(93.5, ...)`처럼 실제 좌표의 100배 근처로 튐). 원인을 실측으로 추적한 결과:

- `Mesh_0` GameObject 자신의 `Transform.localScale`이 `(100, 100, 100)`이다(§25.1에서 이미 확인된,
  블렌더 재수출 과정에서 생긴 보정용 스케일 — 원본 정점 데이터가 1/100로 축소된 채 저장돼 있고,
  평소 렌더링될 때는 이 Transform이 다시 곱해져서 정상 크기로 보이는 구조).
- `SkinnedMeshRenderer.BakeMesh(mesh, useScale:false)`(기본 오버로드)의 결과 정점은 이 100배가
  **이미 적용되지 않은** 상태다(실측: 구운 메쉬의 bounds 크기가 실제 캐릭터와 거의 같은 규모,
  `useScale:true`로 구우면 오히려 100배 더 작아짐 — 확인 결과 이 값은 쓸 수 없음).
- `MeshCollider`는 자신이 속한 GameObject의 Transform을 항상 다시 적용하므로, 이미 100배가 빠진
  "축소되지 않은" 구운 결과를 그대로 대입하면 그 Transform(100배)이 또 한 번 곱해져 이중으로
  커진다.

**수정**: `BakeMesh()` 직후 정점 전체를 `Mesh_0.transform.localScale`의 역수로 나눠 미리
상쇄해뒀다 — GameObject Transform이 다시 곱해지면 정확히 1배로 돌아온다. 회전(§25.13.3에서 확인한
`Armature` 래퍼로 인한 축 방향 차이)은 별도 보정 없이 GameObject 자신의 Transform 회전이 그대로
다시 적용돼 맞는 것을 실측으로 확인했다(스케일만 문제였고 회전은 애초에 정상).

**실측 검증**: 수정 전후 `MeshCollider.bounds`를 `SkinnedMeshRenderer.bounds`(실제 화면 위치)와
비교 — 수정 후 중심이 `(-0.02, 0.93, 0.02)` vs 목표 `(0.00, 0.93, 0.00)`로 사실상 일치함을 확인.

### 20.6-8 [구현 중 신규 발견 ②] 매 프레임 갱신 시 심각한 프레임 드랍 — 스로틀링으로 해결

수정된 로직을 실제로 라운드 진행 중 Play Mode에서 돌려보니, `Time.smoothDeltaTime` 기준 프레임
레이트가 **약 257fps(갱신 없음) → 약 15fps(매 프레임 갱신)로 급락**함을 실측했다. 각 단계를
`Stopwatch`로 개별 프로파일링한 결과:

| 단계 | 비용(30회 평균) |
|---|---|
| `BakeMesh()`(정점 167,404개) | 약 1.46 ms |
| 정점 역스케일 보정 루프(§20.6-7) | 약 1.64 ms |
| `sharedMesh = null` 후 재대입(실제 PhysX 재계산) | 약 3.04 ms |
| (대조군) 같은 참조 그대로 재대입만(재계산 안 됨) | 약 0.0001 ms |

세 단계 합산 약 6ms/프레임만으로도 60fps 기준 프레임 예산(16.7ms)의 상당 부분을 차지하고, 실제
게임 루프에서는 그 이상으로 체감됐다 — `MeshCollider` 재계산(cook) 비용이 압도적으로 크다.

**수정**: `PlayerPaintCanvas`에 프레임 카운터(`colliderRefreshCounter`)를 추가해 **3프레임에 1번만**
`RefreshColliderMesh()`를 실행하도록 스로틀링했다. 붓칠은 마우스를 천천히 움직이며 하는 조작이라
1~2프레임 지연은 체감상 무시할 수준이라고 판단했다(§20.6의 원래 완화 전략과 동일한 방향).

**실측 검증**: 스로틀링 적용 후 라운드 진행 중 프레임 레이트가 **약 15fps → 약 47fps로 개선**됨을
확인했다. 정확도 회귀도 없음을 재확인(가슴/머리/다리 레이캐스트 전부 정상 명중).

### 20.7 부가 조치 — 붓 커서 법선 오프셋 (구현 완료)

`BrushSettingsSO`에 `cursorSurfaceOffset`(기본값 `0.01`) 필드를 추가하고, `BrushCursorController`가
커서를 `hit.point + hit.normal * brushSettings.CursorSurfaceOffset` 위치에 배치하도록 수정했다.
§20.6-7의 스케일 버그를 고친 뒤에는 좌표 불일치 자체가 해소돼 이 오프셋의 실질적 필요성은
낮아졌지만, 사용자가 명시한 "육안으로 붓이 보여야 한다"는 요구사항에 대한 방어적 여유값으로
그대로 유지했다.

### 20.8 검증 계획 — 전부 실행 완료

1. ~~`read_console`로 컴파일 에러 0건 확인~~ → **완료**, 매 코드 변경 직후 확인, 전부 0건.
2. ~~가슴/머리/다리/발 좌표 레이캐스트 재실행~~ → **완료**. 수정 전: 가슴/머리 `hit=False`. 수정
   후: 가슴 `uv=(0.77,0.13)`, 머리 `uv=(0.83,0.03)`, 다리 `uv=(0.10,0.61)` 등 전부 명중.
3. ~~`sharedMesh` 재할당 방식 확정~~ → **완료**(§20.6-5, null 토글 필수로 확정).
4. Walk/Run/Jump/Dodge 등 다른 포즈에서의 재확인 — Idle 포즈 기준으로는 완전히 검증했으나, 다른
   애니메이션 상태에서 실시간으로 쏴보는 것까지는 이번 세션에서 반복하지 않았다(로직 자체가 포즈에
   무관하게 매 프레임 새로 굽는 방식이라 구조적으로는 안전하나, 육안 기준 완전한 확인은 사용자
   플레이 테스트 권장).
5. ~~로컬 전용 확인~~ → 코드 구조상 `pv.IsMine` 가드 뒤에서만 실행되므로 구조적으로 보장됨(기존
   패턴 재사용, 별도 실측 불필요).
6. ~~프레임 드랍 확인~~ → **완료**(§20.6-8, 15fps→47fps 실측 개선).
7. 붓 커서가 몸 표면 위에 뜨는 것처럼 보이는지 육안 확인 — 좌표 자체는 정확해졌음을 실측했으나,
   실제 3D 모델이 화면에 시각적으로 어떻게 보이는지는 이 자동화 환경에서 확인 불가 — **사용자
   플레이 테스트로 최종 확인 필요**.
8. ~~색칠이 상체 포함 전신에서 정상적으로 되는지~~ → **완료**. 실제 `ApplyStamp()`를 가슴 UV
   위치에 호출해 `RenderTexture` 픽셀을 직접 읽어 의도한 색(알파=1)이 정확히 칠해짐을 확인했다
   (§20.9).

### 20.9 상태

**✅ 구현 완료, 실측 검증 완료.** 계획했던 A안(콜라이더 실시간 베이킹)에 더해, 구현 과정에서 새로
발견된 두 가지 문제(§20.6-7 스케일 이중 적용, §20.6-8 프레임 드랍)까지 실측으로 확정하고 함께
해결했다. 컴파일 에러 0건, 콘솔 경고는 기존부터 있던 무관한 항목 1건만 유지. 남은 것은 마우스 육안
조작 기반 최종 확인(§20.8-7)뿐 — 사용자 플레이 테스트를 권장한다.

---

## 21. ⑫ Dodge(회피) 중 캐릭터가 공중에 뜬 것처럼 보이는 버그 — 🔎 원인 확정, B안 구현했으나 롤백됨 — 다른 방안 모색 필요 (2026-08-17)

> 사용자 보고: "Dodge 할 때 1,2,3,7,8,9,10,11,12 프레임때 공중에 떠있는 현상이 있어." Unity MCP로
> Play Mode에서 `Time.timeScale=0`으로 고정하고 회피 동작을 프레임 단위로 재생하며 실제 뼈대 좌표를
> 추적해 원인을 실측으로 확정했다. **이번에는 계획만 정리했고 실제 수정은 진행하지 않았다.**

### 21.1 조사 방법

이전 시도(§16.3 등)와 달리, 이번엔 실제 게임 루프가 도구 호출 사이의 실제 시간 동안 계속 진행되며
테스트 상태를 오염시키는 문제를 먼저 발견하고 `Time.timeScale=0`으로 고정한 뒤, `HideOrSeekPlayer`의
`FixedUpdate()`/`Animator.Update()`를 리플렉션으로 직접, 한 번의 스크립트 실행 안에서 연속
호출하는 방식으로 재측정했다 — 매 스텝마다 다음을 함께 기록했다:
- `rb.position.y`(물리 루트/캡슐의 실제 Y좌표 — 지면에 붙어있는지)
- `Armature/mixamorig:Hips`의 월드 Y좌표(골반 뼈 위치)
- `LeftFoot`/`RightFoot`의 월드 Y좌표(발이 실제로 지면에서 뜨는지 — 가장 직접적인 증거)
- `AnimatorStateInfo.normalizedTime`(회피 애니메이션 진행률)

### 21.2 실측 결과

**루트(물리)는 완전히 정상 — 뜨는 것은 순수하게 시각적(메시) 문제다**: 회피 시작부터 끝까지
`rb.position.y`는 `0.0000`에서 단 한 번도 벗어나지 않았다. 즉 캐릭터의 실제 충돌체/물리 위치는
줄곧 지면에 완벽히 붙어 있다 — 벽이나 바닥을 뚫고 지나가는 등의 물리적 이상은 없다.

**발(뼈대) 좌표는 실제로 크게 떠오른다**: 같은 구간에서 두 발 중 낮은 쪽의 월드 Y좌표
(`minFoot.y`)를 추적한 결과:

| 프레임(≈1/30초 간격) | normalizedTime | minFoot.y(지면 기준 높이, m) |
|---|---|---|
| 2 | 0.200 | 0.170 |
| 3 | 0.267 | 0.320 |
| 4 | 0.333 | **0.776** |
...
| 12 | 0.867 | 0.079 |
| 13 | 0.933 | 0.020 |
| 14 | 1.000 | 0.001(정상, 착지 포즈) |

발이 최대 **1.28m**(캐릭터 키의 절반 이상)까지 지면에서 떠오른다 — "공중에 떠있는 현상"이라는
사용자 보고와 수치로 정확히 일치한다. (사용자가 언급한 정확한 프레임 번호 1,2,3,7,8,9,10,11,12와
위 표의 인덱싱이 1:1로 맞아떨어지진 않을 수 있다 — 카운팅 기준점/에디터상 표시 방식 차이로
보이며, 재현 환경에 따라 오프셋이 있을 수 있다는 점은 감안해야 한다. 다만 "회피 동작 대부분의
구간에서 발이 지면에서 뜬다"는 핵심 증상 자체는 위 실측으로 명확히 재현·확정됐다.)

### 21.3 근본 원인 — Dodge 소스 모션캡처가 "제자리 회피"가 아니라 실제 "전방 다이빙 롤"이다

`Cookie_Dodge_Retimed.anim`의 휴머노이드 루트 모션 커브(`RootT.x/y/z`, `RootQ.x/y/z/w` — Mecanim이
휴머노이드 캐릭터의 "몸 전체 이동/회전"을 표현하는 표준 커브)를 직접 조회한 결과:

| 커브 | 구간 내 변화 | 의미 |
|---|---|---|
| `RootT.y`(수직 높이) | `0.848 → 0.394(최저, t≈0.29~0.32) → 0.848` | 몸 전체가 크게 내려앉았다 다시 솟아오름 |
| `RootT.z`(전진 거리) | `0.0 → 4.08`(끝까지 계속 증가) | **캐릭터가 제자리가 아니라 실제로 앞으로 약 4m 이동하는 모션**으로 캡처됨 |
| `RootQ.w`(회전) | `0.86 → -0.86`(부호가 뒤집힘, 사실상 전신이 한 바퀴 가까이 회전) | **몸 전체가 회전하는 진짜 "공중제비(다이빙 롤)" 동작** |

즉 `Cookie_Dodge`는 단순한 "옆으로 슬쩍 피하는 스텝"이 아니라, **실제 모션캡처 배우가 앞으로
몸을 던져 구르는(다이빙 롤) 전신 동작**이며, 원래는 이 루트 모션(위치+회전)을 게임 오브젝트에
그대로 적용해서 캐릭터가 실제로 굴러가도록 설계된 클립으로 보인다.

그런데 `HideOrSeekPlayer.Start()`(§25.13.2)는 `animator.applyRootMotion = false`를 명시적으로
설정한다 — 이동은 전부 `Move()`가 `Rigidbody.linearVelocity`를 직접 갱신하는 방식이라, 클립에
내장된 루트 모션이 게임 오브젝트의 실제 Transform에는 **전혀 반영되지 않는다.** 하지만
Mecanim은 `applyRootMotion`이 꺼져 있어도 스켈레톤 전체를 이 루트 커브 기준으로 계속 포즈
계산한다 — 그 결과 **게임 오브젝트(캡슐)는 완벽히 고정된 채, 그 위에 얹힌 스켈레톤(메시) 전체만
"제자리에서 위아래로 크게 오르내리고 앞뒤로 쏠리고 통째로 회전"** 하는 모습으로 나타난다. 이것이
"발이 공중에 뜬 것처럼 보인다"는 증상의 정확한 메커니즘이다.

**참고**: 재타이밍(§25.13.4)은 키프레임의 `time`만 압축하고 `value`는 그대로 복사하므로, 이 문제는
재타이밍 자체가 만든 것이 아니라 원본 `Cookie_Dodge.fbx` 모션캡처 소스에 원래부터 있던 특성이다
(원본도 같은 값 패턴을 그대로 갖고 있고, 다만 1.633초에 걸쳐 펼쳐져 있을 뿐이다). Idle/Walk/Run/Jump
클립은 §25.13.6 검증 당시 `rb.position.y`가 정상적으로 유지됨을 이미 확인했고, 이번 조사로 Dodge만
갖는 특이 케이스임이 재확인됐다.

### 21.4 수정 방안 (3가지, 장단점 비교)

| 방안 | 설명 | 장점 | 단점 |
|---|---|---|---|
| **A. `RootT.y`만 정규화(수직 높이 고정)** | 재타이밍 파이프라인에 후처리를 추가해 `RootT.y` 커브를 0번 키프레임 값 기준으로 상대 보정(또는 아예 평평하게 고정)해 "위아래로 붕 뜨는" 성분만 제거 | 구현 간단(§25.13.4의 커브 조작 코드 확장), 코드/물리 변경 없음 | `RootQ`(전신 회전)는 그대로 남아 캐릭터가 "제자리에서 몸이 뒤집히는" 듯한 다른 어색함이 남을 수 있음(육안 확인 필요) |
| **B(권장) `RootT`(x/y/z 전부) + `RootQ` 전부 정규화 — 순수 "제자리" 애니메이션으로 변환** | 루트 위치·회전 커브를 전부 0번 키프레임 기준으로 고정/상쇄해, 팔다리·척추 등 루트에 상대적인 로컬 모션만 남긴다 | 가장 안전하고 예측 가능 — 뜨는 것도, 제자리에서 뒤집히는 것도 전부 제거. `applyRootMotion=false`라는 기존 설계 원칙(§12.7)과 가장 잘 맞음 | 다이빙 롤 특유의 "몸을 던지는" 화려함이 사라지고, 팔다리 로컬 움직임만 남아 밋밋한 회피 동작으로 보일 위험(원본 모션의 개성 대부분이 루트 모션 쪽에 있었기 때문) — 실제 결과물은 적용 후 육안 확인 필요 |
| **C. 부분적 루트 모션 수동 적용 — Dodge 중에만 `animator.deltaPosition`/`deltaRotation`을 코드로 직접 반영** | `applyRootMotion`은 계속 꺼둔 채, `HideOrSeekPlayer.FixedUpdate()`가 `isDodge`일 때만 매 프레임 `animator.deltaPosition`/`deltaRotation`을 읽어 `Rigidbody`에 수동으로 더해줌(Y축만 선택적으로 반영하거나 전부 반영) | 모션캡처가 원래 의도한 "실제로 몸을 던져 구르는" 느낌을 최대한 살릴 수 있음 | 구현 복잡도가 가장 높음 — 기존 `dodgeMoveDir` 기반 수평 이동 로직과의 통합, 충돌/낙사/다른 플레이어와의 상호작용 안전성, 회피 중 벽을 뚫고 구르는 것 방지 등을 전부 새로 설계해야 함. 이번 회피 시스템의 "0.5초 대시 슬라이드"라는 기존 설계 의도(§14.3)와도 재조율 필요 |

**결정**: **B안(루트 모션 전체 정규화)**을 적용했다. 사용자가 "혹시 모를 사항을 대비하여 백업본을
만들어놓고, B안으로 수정후 구현해줘"라고 명시적으로 확정 지시함.

### 21.5 B안 구현 결과

계획(§21.5 원안)은 "재타이밍 파이프라인에 보정 단계를 추가해 `new AnimationClip()`으로 재생성"하는
방식을 제안했었으나, 실제로는 **더 간단하고 안전한 방법으로 구현했다** — 이미 `length=0.5`로
정상 동작 중인 기존 `Cookie_Dodge_Retimed.anim` 에셋을 그대로 두고, 그 7개 루트 커브만 **직접
덮어쓰는(in-place `SetEditorCurve`)** 방식을 썼다. 이렇게 하면 §25.13.4에서 겪었던 "새 클립을
만들면 guid가 바뀌어 Animator Controller 재연결이 필요한" 문제 자체가 발생하지 않는다.

1. **백업 생성**: 수정 전 `Cookie_Dodge_Retimed.anim`을 `AssetDatabase.CopyAsset()`으로
   `Assets/Animation/Cookie/Cookie_Dodge_Retimed_Backup_PreRootFix.anim`에 그대로 복사해뒀다
   (`length=0.5` 동일 확인).
2. **정규화 방식 — "0번 기준 상대값"이 아니라 "0번 값으로 완전히 고정(flatten)"으로 구현**:
   계획 초안은 "0번 키프레임 값을 뺀 상대값"이라고 적었으나, 그 방식은 0번 프레임만 0으로
   정렬될 뿐 **중간 구간의 요동(§21.2에서 실측한 뜨는 원인)은 그대로 남는다** — 목표(뜨는 현상
   자체를 없애는 것)에 맞지 않아, 실제 구현 시점에 **7개 루트 커브 전부를 각자의 0번 시점 값으로
   완전히 고정(2개 키프레임, 동일한 값, Linear 탄젠트)** 하는 방식으로 정정해 구현했다. 이 방식은
   §21.4 B안 설명("루트 위치·회전 커브를 전부 고정/상쇄")의 원래 취지와 정확히 일치한다.
   - `RootT.x` → `0.05857`로 고정, `RootT.y` → `0.84787`로 고정, `RootT.z` → `0.03753`로 고정
   - `RootQ.x/y/z/w` → 각각 `0.14523 / 0.48409 / -0.06946 / 0.86008`로 고정 — 4개 성분 전부를
     독립적으로 자신의 0번 값에 고정하면 조합된 쿼터니언도 자동으로 항상 `Q(0)`으로 일정하게
     유지되므로, 굳이 `Quaternion.Inverse(q0) * qi`로 매 프레임 재계산할 필요가 없었다(4개 성분
     커브가 서로 다른 키프레임 시각을 갖고 있어 원래 계획대로 하려면 공통 시간축으로 리샘플링해야
     했는데, 완전 고정 방식은 그 복잡도 자체가 필요 없다).
3. **나머지 123개 커브(팔다리/척추 등 근육 커브)는 전혀 건드리지 않았다** — 로컬 관절 모션은
   그대로 보존됨.
4. **컨트롤러 재연결 불필요**: 같은 파일을 in-place 수정했으므로 guid가 바뀌지 않았고,
   `PlayerAnimator.controller`의 `Dodge` 상태가 여전히 같은 에셋(`Cookie_Dodge_Retimed`,
   `length=0.5`)을 정상 참조함을 재조회로 확인했다.

### 21.6 검증 결과

1. ~~`read_console`로 컴파일 에러 0건~~ → **완료.** 에셋 수정 전/후 전부 에러 0건, 기존부터 있던
   무관한 경고 1건만 유지.
2. ~~§21.1과 동일한 방식으로 재측정~~ → **완료, 극적으로 개선됨.**

   | 지표 | 수정 전(§21.2) | 수정 후 |
   |---|---|---|
   | 발의 최고 높이(`minFoot.y` 최대값) | **1.278 m** | **0.375 m** |
   | 골반(Hips) 높이 변동 범위 | `0.618 ~ 0.945`(폭 0.33m) | `0.611 ~ 0.766`(폭 0.15m) |
   | `rb.position.y`(물리 루트) | `0.000`(정상) | `0.000`(정상, 회귀 없음) |

   발이 완전히 `0`까지 떨어지진 않지만(최대 0.375m, 대략 무릎 높이), 이는 **구르기 동작 특유의
   무릎을 웅크리는 로컬 다리 관절 모션**(§21.5-3에서 보존한 123개 커브에 포함)이 남아있기 때문 —
   실제 사람이 구를 때도 다리를 굽히면서 발이 잠깐 지면에서 뜨는 것은 정상적인 동작이다. 이전의
   "가슴 높이(1.28m)까지 통째로 붕 뜨는" 비정상적인 증상과는 명확히 구분된다.
3. ~~`rb.position.y` 회귀 확인~~ → **완료.** 위 표 참고, 전 구간 `0.000` 유지.
4. 회피 애니메이션의 시각적 자연스러움 — 수치상 개선은 명확히 확인했으나, "육안으로 봤을 때
   자연스러운가"는 이 자동화 환경에서 최종 판단할 수 없다 — **사용자 플레이 테스트로 최종 확인
   필요**(§21.6-4는 여전히 이 세션 범위 밖).
5. ~~0.5초 완료 시점 회귀 확인~~ → **완료.** `normalizedTime`이 이전과 동일하게 프레임 14 부근에서
   `1.000`에 도달, 재타이밍 자체는 전혀 영향받지 않았음을 재확인(시간축은 건드리지 않았으므로
   예상대로).
6. ~~Jump/Walk/Run/Idle 회귀 확인~~ → **완료.** 4개 상태 전부 `IsName=True`, 각자의 정상 `length`
   (8.333/1.2/0.5333/1.9) 유지 — Dodge 클립 하나만 수정했으므로 예상대로 영향 없음.

### 21.7 상태 — B안 구현했으나 사용자 요청으로 롤백됨 (2026-08-17)

B안(루트 모션 7개 커브를 0번 프레임 값으로 완전히 고정)을 실제로 구현·검증까지 완료했었다 — 발의
최고 높이가 `1.28m → 0.375m`로 수치상 크게 개선됐고, 물리 루트·다른 애니메이션 상태·재타이밍
완료 시점 전부 회귀 없음을 확인했었다. 그런데 사용자가 실제 결과를 확인한 뒤 "방금 했던 작업을
다시 되돌려줘 다른 방안이 필요한거같아"라고 요청해, **§21.5의 구현을 백업 파일로 완전히 되돌렸다**:

- `Cookie_Dodge_Retimed.anim`의 7개 루트 커브를 `Cookie_Dodge_Retimed_Backup_PreRootFix.anim`의
  원본 데이터로 다시 덮어썼다(같은 파일을 in-place 복원, guid 불변이라 `PlayerAnimator.controller`
  재연결 불필요 — 재조회로 `Dodge` 상태가 여전히 정상 참조함을 확인).
- 복원 후 `RootT.y` 범위가 `[0.375, 0.848]`로 원래의 변동폭을 다시 보이는 것을 재조회로 확인 —
  §21.2에서 실측했던 "발이 최대 1.28m까지 뜨는" 원래 상태로 정확히 돌아갔다.
- `read_console` 컴파일 에러 0건, 기존 무관 경고 1건만 유지.

**정확히 어떤 점이 마음에 안 들었는지(뜨는 건 줄었지만 동작 자체가 밋밋해 보였는지, 다른 시각적
문제가 새로 생겼는지 등)는 아직 전달받지 못했다** — §21.4에서 검토했던 C안(Dodge 중에만
`animator.deltaPosition`/`deltaRotation`을 코드로 부분 반영해 실제로 몸을 던지는 느낌을 살리는
방향) 또는 완전히 다른 방향(예: 소스 애니메이션 자체를 교체) 중 어느 쪽을 원하는지 사용자 확인
후 다음 계획을 이어서 정리할 예정이다. `Cookie_Dodge_Retimed_Backup_PreRootFix.anim`은 참고용으로
그대로 남겨뒀다.
>
> **⑬(신규, 2026-09-25 추가, 같은 날 구현·검증까지 완료)** — `Monster-Rerig-Plan.md`로 몬스터를
> 새 리깅·애니메이션으로 교체한 직후, 사용자가 "몬스터가 땅으로 박힌다"고 제보했다(첨부
> 스크린샷은 이 세션에 실제 도달하지 않아 좌표 실측으로 대신 확인). 1차 수정안(`CapsuleCollider.
> center.y`를 `-1.225`로 옮겨 캡슐 바닥을 메시 최하단에 맞춤)은 적용 즉시 Play Mode 검증에서
> **캐릭터가 바닥을 완전히 뚫고 무한 낙하하는 더 심각한 새 버그**를 일으키는 것을 실측
> (`root.y=-3463`, `velocity.y=-260`)으로 확인해 즉시 롤백했다 — 원인은 콜라이더를 발밑까지
> 내리면서 `Ground` 콜라이더(두께 3유닛)의 범위 밖으로 나가버렸기 때문. 재설계한 최종안은
> **모델(`Mesh_0`/`MonsterArmature`) 자체를 루트 기준 +1.4037 들어올려 피벗이 발밑에 오도록
> 재배치**하고 콜라이더·`EyeSocket`을 새 위치에 맞춰 재계산하는 것 — 재측정으로 메시-콜라이더-
> 지면 간극이 사실상 0임을 확인했고, Play Mode에서 낙하도 파묻힘도 재현되지 않음을(정지 시
> `velocity.y=0`) 확인해 **해결된 것으로 보고했었다.** `GrabKill` 트리거 위치는 이 수정의 부작용으로
> 재조정 필요성이 생겨 별도 후속 과제로 남겼다.
>
> **⑬ 후속 재조사(같은 날, 2026-09-25) — 위 "완전히 해결됐다"는 결론 자체가 틀렸음이 드러남** —
> 사용자가 이어서 첨부한 스크린샷(Prefab 편집 모드에서 `SphereCollider`가 몸통 중앙에 떠 있는
> 모습)을 보고 "이러면 지면에 항상 붙어있지 않고 떨어져 있는 현상이 있을 것 같다"고 지적했다.
> 재조사 결과 지적이 정확했다 — 위에서 검증에 썼던 `SkinnedMeshRenderer.bounds`가 애니메이션
> 재생 중 실제 스킨 변형을 반영하지 않는 근사값(본 하나의 트랜스폼만 따라감)이었고,
> `BakeMesh()`로 정점 단위로 다시 측정한 결과 Idle/Walk/TentacleDash/GrabKill 4개 상태 전부
> 진짜 포즈에서는 캡슐 바닥보다 약 3.06~3.07유닛 위에 떠 있었다(§20에서 `PlayerPaintCanvas`가
> 이미 겪었던 것과 같은 함정에 이번에도 걸린 것). 모델을 추가로 `-0.565` 더 내려 3차 수정하고
> `BakeMesh` 기반으로 재검증한 결과 4개 상태 전부 간극이 ±0.003 이내로 수렴, Play Mode에서도
> 완전히 정지함을 확인해 **이번에는 신뢰할 수 있는 방법으로 최종 해결됐다.** 상세는 §22.8~22.10.

---

## 22. ⑬ 재리깅된 몬스터가 땅에 박혀 보이는 버그 — ✅ 원인 실측 확정·구현·검증 완료 (2026-09-25, §22.8~22.10 최종 수정안으로 완료)

> **§22.3의 1차 수정안(캡슐만 `center.y=-1.225`로 이동)은 실제 적용 후 Play Mode 검증에서
> 더 심각한 새 버그(캐릭터가 바닥을 완전히 뚫고 무한히 낙하)를 일으키는 것으로 드러나 즉시
> 롤백하고 재설계했다.** 2차로 채택한 방식(§22.5)은 콜라이더가 아니라 **모델(시각적 몸체)
> 자체를 루트 피벗 기준으로 들어올려 피벗=발밑이 되도록 재배치**하는 것이었는데, **이 2차
> 수정도 검증 방법(`SkinnedMeshRenderer.bounds`)이 실제 애니메이션 포즈를 반영하지 못하는
> 근사값이었던 탓에 "완료"로 잘못 보고됐었다** — 사용자가 스크린샷을 보고 재차 지적해
> `BakeMesh()` 기반으로 재검증한 끝에 3차 수정(§22.8~22.10)으로 최종 해결했다. 아래 §22.1~22.4는
> 최초 조사 당시의 원인 분석과 1차 수정안을, §22.5~22.7은 (나중에 불충분함이 드러난) 2차
> 수정안을 그대로 보존하고, §22.8~22.10에 실제로 최종 적용된 3차 수정·검증 결과를 정리했다.

> 사용자가 스크린샷("땅에 박히는 버그.png")을 언급하며 제보했으나, 이 세션에는 이미지 파일 자체가
> 텍스트로만 전달되고 실제 바이너리는 도달하지 않았다(첨부 재확인 요청함). 대신 `Monster-Rerig-
> Plan.md`로 방금 재구성한 `MonsterPlayer.prefab`을 Unity MCP `execute_code`로 직접 인스턴스화해
> 좌표를 실측하는 방식으로 원인을 확정했다 — 스크린샷 없이도 수치로 100% 재현되는 구조적 문제다.

### 22.1 증상

몬스터 캐릭터가 바닥(`Ground`)에 서 있을 때, 시각적으로 몸체 하단이 지면 아래로 파묻힌 것처럼
보인다(사용자 표현: "땅으로 박히는 오류").

### 22.2 원인 — `CapsuleCollider`가 새 모델의 실제 몸체 범위와 맞지 않음

`Monster-Rerig-Plan.md` §10.3-7에서 캡슐 콜라이더를 "스케일 차이가 10%뿐이라 로컬 수치를 그대로
유지"하기로 했던 판단이 **불충분했다** — 스케일(전체 크기)은 구 모델과 비슷하게 맞았지만,
**루트 피벗을 기준으로 한 콜라이더의 세로 위치(collider의 상대적 높이)까지는 검증하지 않았다.**
실제로 `PrefabUtility.InstantiatePrefab`으로 현재 프리팹을 인스턴스화해 직접 측정한 결과:

| 항목 | 값 (루트 로컬 좌표 기준) |
|---|---|
| `SkinnedMeshRenderer` 최하단(발/몸체 바닥) | **Y = -1.40** |
| 현재 `CapsuleCollider` 바닥(`center.y - height/2`) | **Y = -0.025**(`center=(0,0.15,0)`, `height=0.35`) |
| 두 값의 차이 | **약 1.375 로컬 단위**(현재 스케일 5.42 기준 월드로 약 **7.45 유닛**) |

`MonsterController.Start()`가 `rb.useGravity = true`로 세팅하므로, 스폰 후 물리 시뮬레이션은
**`CapsuleCollider`가 지면에 닿는 지점**에서 캐릭터를 멈춘다 — 즉 캡슐 바닥(로컬 -0.025)이 지면
높이(Y=0)에 오도록 루트가 정지한다. 그런데 실제로 눈에 보이는 메시(발/몸체 바닥)는 루트보다
**1.40 로컬 단위 더 아래**에 있으므로, 캡슐이 지면에 정상적으로 걸쳐 있어도 **눈에 보이는 몸체는
지면보다 한참 아래(월드 기준 약 -7.47유닛)에 파묻힌 것처럼 렌더링된다.**

근본 원인은 `Monster_Rigged_kihong`의 로컬 원점(피벗)이 구 모델(`Monster_Rigged`)과 마찬가지로
**몸체 중심부 부근에 있고 발밑에 있지 않다**는 점이다(§10.1에서 확인했듯 `Head` 본의 로컬 좌표가
`(0, 0.25, 0.03)`으로 원점에 가까운 것과 같은 맥락 — 원점이 몸통 한가운데 근처에 있다). 이
자체는 구 모델도 동일했지만(§10.3-3에서 실측한 구 모델도 메시 최하단이 루트 기준 약 -1.14 로컬
수준으로 추정되는 큰 오프셋을 가짐), **캡슐 콜라이더의 `center.y` 값(0.15)을 재계산 없이 그대로
재사용**한 것이 이번 재리깅에서 처음으로 가시화된 구체적 실수다 — 계획 문서(`Monster-Rerig-Plan.md`
§5-5)가 애초에 "Scene 뷰 Gizmo로 캡슐이 실제 모델 몸통을 덮는지 확인"하라고 명시했던 절차를
실행 단계에서 스킵하고 "스케일 비율이 비슷하니 안전하다"고만 판단한 것이 원인이다.

### 22.3 수정 계획 (1차안 — ⚠️ 적용 후 새 버그 유발, §22.5에서 폐기)

`Assets/04. Prefabs/Resources/MonsterPlayer.prefab`의 `CapsuleCollider`(루트) `center.y`를
재계산한다. 반경(`radius=0.1`)·높이(`height=0.35`)는 그대로 두고 **`center.y`만 이동**해 캡슐
바닥이 실제 메시 최하단과 맞도록 한다:

```
desiredBottomLocal = -1.40   (실측값, §22.2)
height = 0.35                (변경 없음)
newCenter.y = desiredBottomLocal + height/2 = -1.40 + 0.175 = -1.225
```

즉 `CapsuleCollider.center`를 `(0, 0.15, 0)` → **`(0, -1.225, 0)`** 로 변경한다(x/z는 그대로).

Unity MCP로는 `PrefabUtility.LoadPrefabContents` → `CapsuleCollider.center = new Vector3(0f,
-1.225f, 0f)` → `PrefabUtility.SaveAsPrefabAsset`으로 처리 가능(코드 변경 없음, 프리팹 데이터만
수정).

**함께 재확인이 필요한 것**: `SphereCollider`(GrabKill 트리거, 현재 `center=(0, 0.15, 0.1)`)도
같은 피벗-오프셋 문제를 공유한다 — 캡슐만 고치면 캐릭터는 지면에 정상적으로 서지만, GrabKill
판정 범위는 여전히 몸통 중간 높이(지면에서 한참 뜬 위치)에 머무른다. 이번 버그(그라운딩)의 직접
원인은 아니므로 범위를 분리하되, 캡슐 수정과 같은 세션에서 함께 처리하는 것을 권장 — 정확한
목표 높이(적/쿠키 몸통 높이 기준)는 실측이 아니라 게임 디자인 판단이 필요해 사용자 확인 후
반영한다.

### 22.4 검증 계획 (1차안 기준— 실행 결과는 §22.5 참고)

1. 수정 직후 `PrefabUtility.InstantiatePrefab`으로 재인스턴스화해 `CapsuleCollider` 바닥
   local Y와 `SkinnedMeshRenderer` 최하단 local Y가 (오차 범위 내에서) 일치하는지 재측정.
2. `PlayerTestScene`에서 `OfflineModeBootstrap`(`autoCreateRoom`/`spawnAsMonster`=true, 테스트
   후 원복)으로 Play Mode 진입 → 스폰된 몬스터가 지면 위에 자연스럽게 서 있는지(메시 최하단이
   `Ground` 표면과 맞닿는지) 확인.
3. `read_console`로 에러/경고 0건 확인.
4. 회귀 확인: `MonsterController`의 이동·`TentacleDash`·`GrabKill` 트리거 판정이 캡슐 위치 변경
   이후에도 기존과 동일하게 동작하는지(캡슐이 아래로 이동했으므로 벽 충돌 높이 등이 달라질 수
   있음 — 벽 통과/걸림 여부 재확인).
5. 가능하면 사용자가 실제 스크린샷으로 "땅에 박힘" 현상이 사라졌는지 최종 확인(§22 서두에서
   언급한 첨부 이슈로 이번 세션에서는 시각적 대조를 직접 하지 못함).

**실행 결과(1번 항목만 적용해본 결과) — 계획대로 §22.4-1은 통과했으나 §22.4-2에서 새 버그
발견**: `execute_code`로 실측한 결과 캡슐 바닥과 메시 최하단의 world 간극이 7.47유닛 → 0.02유닛
으로 줄어 §22.4-1 기준으로는 "성공"이었다. 그런데 §22.4-2(Play Mode 실제 스폰)를 진행한 결과,
`Time.time≈26.5s` 시점에 `root.y=-3463`, `velocity.y=-260`로 **캐릭터가 바닥을 완전히 뚫고
끝없이 낙하하는 것을 실측으로 확인**했다 — §22.5에서 원인을 재분석한다.

### 22.5 재조사 — 왜 1차 수정안이 바닥을 뚫는 새 버그를 만들었는가, 그리고 진짜 수정

**원인**: `MonsterSpawnPos`는 지면(`Ground` 콜라이더 top, world Y=0)과 같은 높이(Y=0)에 있다.
`PhotonNetwork.Instantiate`는 이 좌표에 **루트를 그대로** 배치하므로, "루트가 지면 높이에 있을 때
메시 발밑도 지면과 맞아야 정상"이라는 전제가 깔려 있다. 그런데 §22.2에서 실측했듯 **메시
최하단은 루트보다 로컬 -1.40(월드 약 -7.6) 아래에 있다** — 즉 루트 자체가 이미 "몸통 한가운데
근처"에 있고 지면 위치와는 원래 아무 관계가 없다.

1차 수정안은 "콜라이더 바닥을 메시 최하단과 맞춘다"는 것까지는 맞았지만, 그 결과 **콜라이더
자체가 루트 기준 로컬 -1.40(월드 -7.6)까지 내려갔다.** `Ground` 콜라이더는 `Center: (0, -1.5, 0),
Extents: (36, 1.5, 36)` — 즉 두께가 3유닛뿐이라 **월드 Y -3~0 범위에만 존재한다.** 루트가
`MonsterSpawnPos`(Y=0)에 스폰되는 순간 콜라이더는 이미 월드 Y -7.6 부근에 있어 **지면 슬래브
(-3~0) 밑에서 시작**한다 — 즉 스폰 시점에 이미 지면과 전혀 겹치지 않으므로 물리 엔진이 충돌을
감지할 기회 자체가 없고, 그대로 중력에 의해 영원히 낙하한다(`ContinuousDynamic`이 막는 것은
"고속 이동 중 얇은 콜라이더를 관통하는 것"이지, "애초에 접촉이 없던 상황"은 막지 못한다).

**결론**: 콜라이더 위치만 옮기는 것은 대증(對症) 요법이었다 — 진짜 문제는 **모델의 로컬 원점
(피벗)이 발밑이 아니라 몸통 중앙 부근에 있다**는 것 자체다. 콜라이더를 발밑으로 옮기면 "메시와
콜라이더는 맞아떨어지지만 루트(=스폰 좌표계 기준점)와는 안 맞는" 문제로 형태만 바뀔 뿐이다.
**진짜 수정은 모델(시각적 몸체) 자체를 루트 기준으로 들어올려, 루트 원점이 실제로 발밑(지면
접촉면)에 오도록 재배치하는 것**이다 — 이렇게 하면 "루트를 지면 높이에 놓으면 발도 지면에 닿는다"
는 자연스러운 전제가 성립하고, 콜라이더도 원래 있던 자리(루트 부근, 원래 `center.y=0.15`와 거의
같은 값)로 되돌아가 `Ground` 콜라이더의 3유닛 두께 안에 다시 들어오므로 §22.5의 낙하 버그도
자동으로 해소된다.

**적용한 수정** (`Assets/04. Prefabs/Resources/MonsterPlayer.prefab`, `PrefabUtility.
LoadPrefabContents`로 헤드리스 편집):

1. 임시 인스턴스로 메시 최하단의 루트-로컬 Y를 재측정: **-1.403702**(§22.2와 동일값, 재확인).
2. `Mesh_0`/`MonsterArmature` 두 자식의 `localPosition.y`를 `0` → **`+1.403702`**로 이동(모델
   전체를 위로 들어올려 발밑이 로컬 Y=0, 즉 루트 원점과 일치하도록 재배치).
3. 재측정 결과 메시 최하단이 로컬 `1.76e-7`(사실상 0)로 이동한 것을 확인 후, `CapsuleCollider.
   center`를 `(0, -1.225, 0)`(1차안의 잘못된 값) → **`(0, 0.18, 0)`**(`= 새 메시 최하단 + height/2`,
   반경/높이는 그대로 0.1/0.35)로 재조정 — 결과적으로 §22.3 이전 원래 값(0.15)과 거의 비슷한
   수치로 복귀했다(모델을 들어올렸으니 당연한 결과).
4. `EyeSocket`도 새 `Head` 본 위치(로컬 `(0, 1.65, 0)`, 이전 `(0, 0.25, 0.03)`에서 위로
   1.40 이동)를 기준으로 재계산 + 기존과 동일한 전방 0.03 오프셋 → 최종 `(0, 1.65, 0.03)`.

### 22.6 검증 결과 (최종안)

1. **재측정(정적)**: 메시 최하단 로컬 Y `1.76e-7`, 캡슐 바닥 로컬 Y `1.79e-7` — **간극
   `-1.5e-8`(부동소수점 오차 수준, 사실상 완전 일치)**. `Ground` 두께(-3~0) 안에 캡슐이 확실히
   들어옴.
2. **Play Mode 재검증** (`PlayerTestScene`, `OfflineModeBootstrap.autoCreateRoom`/
   `spawnAsMonster`=true로 재테스트, 종료 후 원복): 스폰 `t=1.12s` 시점에 `root.y≈0`,
   `meshMin.y≈0.106`(지면보다 살짝 위, 파묻히지도 뚫지도 않음), `rb.linearVelocity.y=0`(완전히
   정지) — **낙하 버그도, 파묻힘 버그도 재현되지 않음.**
3. **애니메이션 회귀 확인**: `Idle`/`Walk`/`TentacleDash`/`GrabKill`(`GrapKill` 상태) 4개 트리거
   전부 `IsName()`으로 정상 전환 확인(모델 위치만 옮겼을 뿐 본 계층·애니메이터 배선은 손대지
   않았으므로 예상대로 회귀 없음).
4. `read_console` — 전 과정 에러/경고 0건(무관한 MCP 브리지 포트 재시도 로그 1건만 확인, 이번
   작업과 무관).
5. **후속 확인 필요 항목(범위 밖, 새로 발생한 부작용)**: `SphereCollider`(GrabKill 트리거,
   `center=(0, 0.15, 0.1)`, 이번에 건드리지 않음)가 모델이 위로 이동한 만큼 **상대적으로 몸통
   기준 더 아래(이제 세계 좌표 Y≈0.81, 발 근처)로 내려간 것처럼 보이게 됐다** — §22.3에서 이미
   "게임 디자인 판단 필요"로 분리해뒀던 항목인데, 이번 수정으로 그 필요성이 더 커졌다. 정확한
   목표 높이(쿠키 몸통 기준 그랩 판정 위치)를 사용자가 정하면 별도로 반영할 것.
6. 사용자가 실제 화면으로 "땅에 박힘" 현상이 사라졌는지 최종 확인 권장(이번 세션은 스크린샷 첨부가
   도달하지 않아 좌표 실측으로만 검증함, §22 서두 참고).

### 22.7 최종 상태 — ⚠️ §22.6의 측정 방식 자체가 부정확했음이 드러나 재수정(§22.8)

### 22.8 [3차 재조사] 사용자 지적으로 발견 — `SkinnedMeshRenderer.bounds`는 실제 애니메이션
포즈를 반영하지 않는 근사값이었다

사용자가 스크린샷("몬스터 콜라이더.png" — Prefab 편집 모드에서 `SphereCollider`가 몸통 한가운데
높이에 있는 모습)을 보고 "이러면 지면에 항상 붙어있지 않고 떨어져 있는 현상이 있을 것 같다"고
지적했다. §22.6까지는 `SkinnedMeshRenderer.bounds`(Idle/Walk/TentacleDash/GrapKill 4개 상태를
`Animator.Play(state, 0, normalizedTime)` + `Animator.Update(0)`로 강제 재생하며 샘플링)로
"최하단 Y가 8~16회 샘플 전 구간에서 정확히 0.0000, 변동 없음"을 확인하고 "애니메이션 중에도
그라운딩이 안정적"이라고 결론 내렸었다.

**이 결론 자체가 틀렸다.** `SkinnedMeshRenderer.bounds`는 실제 GPU 스키닝 결과를 매 프레임
재계산하지 않고(`m_UpdateWhenOffscreen: 0`), **본 하나(주로 루트 계열 본)의 현재 트랜스폼만
반영한 근사값**을 돌려준다 — 즉 텐타클이 크게 흔들리고 머리가 위아래로 들썩여도(직접 본 좌표를
찍어보면 텐타클 끝이 Y `10.54~11.65` 사이를 오가는 것까지 확인됨) `bounds.min.y`는 절대 안
바뀌는 것이다. `PlayerPaintCanvas`가 이미 §20(붓 커서 파고듦 버그)에서 이 문제를 만나
`SkinnedMeshRenderer.BakeMesh()`로 실제 스킨된 정점을 직접 구워 우회했던 것과 정확히 같은
함정에 이번에도 걸린 것 — 이번엔 인지하지 못한 채 넘어갔다가 사용자 지적으로 재발견했다.

`BakeMesh()`로 4개 애니메이션 상태 전 구간(정점 단위, 매 상태 12~16회 샘플)의 **진짜** 최하단
Y를 다시 측정한 결과:

| 상태 | 진짜 최하단 world Y (§22.6 이전 수정 기준, root=0) |
|---|---|
| Idle | 3.0729 |
| Walk | 3.0614 |
| TentacleDash | 3.0630 |
...
4개 상태 전부 **캡슐 바닥(world Y≈0)보다 약 3.06~3.07 유닛 위**에서 거의 일정하게 유지되고
있었다 — 즉 §22.5~22.6의 수정은 **정지된 바인드 포즈(가져오기 시점의 기본 자세) 기준으로는
정확했지만, 실제 게임에서 재생되는 애니메이션 포즈 기준으로는 캐릭터가 지면 위 약 3유닛
공중에 떠 있는 상태**였다 — 사용자가 스크린샷을 보고 예상한 "떨어져 있는 현상"이 정확히
실측으로 확인된 것이다. (참고로 4개 상태 사이의 편차는 0.01 미만으로 매우 작아, "어떤
애니메이션이냐에 따라 바닥과의 거리가 들쭉날쭉해지는" 문제는 아니고 — 4개 상태 전부가 **한
방향으로 똑같이** 바인드 포즈보다 위로 들려있는 것이었다.)

**재수정**: `Mesh_0`/`MonsterArmature`/`EyeSocket`의 `localPosition.y`에 추가로
`-3.061931 / 5.42 = -0.565`를 더해(즉 모델을 조금 더 아래로) 진짜 애니메이션 포즈의 최하단이
캡슐 바닥과 맞도록 재조정했다:

| 오브젝트 | §22.6까지 | §22.8 최종 |
|---|---|---|
| `Mesh_0`/`MonsterArmature`.localPosition | `(0, 1.40, 0)` | **`(0, 0.84, 0)`** |
| `EyeSocket`.localPosition | `(0, 1.65, 0.03)` | **`(0, 1.09, 0.03)`** |
| `CapsuleCollider.center` | `(0, 0.18, 0)` | 변경 없음(`(0, 0.18, 0)`) — 모델만 내렸으므로 캡슐은 그대로 두면 저절로 다시 맞음 |

### 22.9 최종 검증 (`BakeMesh` 기반, 신뢰 가능한 측정)

재수정 후 동일한 방식(4개 상태 × 12~16 샘플, `BakeMesh()`로 정점 단위 실측)으로 재검증:

| 상태 | 진짜 최하단 world Y | 캡슐 바닥 대비 간극 |
|---|---|---|
| Idle | -0.0030 | -0.0030 |
| Walk | -0.0001 | -0.0001 |
| TentacleDash | -0.0033 | -0.0033 |
| GrapKill | -0.0022 | -0.0022 |

4개 상태 전부 간극이 **±0.003 유닛 이내**(밀리미터 단위 오차, 시각적으로 감지 불가능한 수준)로
수렴했다 — 이제 캡슐 바닥과 실제 애니메이션 포즈의 최하단이 거의 완벽하게 일치한다.

Play Mode 물리 재검증: 스폰 후 `t=12.6s` 시점 `root.y≈0`, `rb.linearVelocity.y≈0`(완전 정지,
낙하도 파묻힘도 없음). `read_console` 에러/경고 0건.

### 22.10 최종 상태

✅ **구현·(신뢰할 수 있는 `BakeMesh` 기반) 실측·Play Mode 검증 전부 완료.** 이번 조사로 얻은
교훈: `SkinnedMeshRenderer.bounds`는 스키닝 애니메이션 캐릭터의 실제 포즈를 반영하지 않으므로,
앞으로 이 프로젝트에서 캐릭터의 실제 형상(정점 위치)을 검증할 때는 반드시 `BakeMesh()`를 거쳐야
한다(`PlayerPaintCanvas.RefreshColliderMesh()`가 이미 같은 이유로 이 방식을 쓰고 있음, §20).
