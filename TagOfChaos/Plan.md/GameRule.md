...
> **v3.8 변경(2026-09-26, Bug-fix-plan.md §23 — 구현·검증 완료)**
> - **괴물 카메라: 1인칭 → 3인칭으로 변경(사용자 결정, A안).** 쿠키와 같은 `Camera_Ctrl` 궤도 카메라(우클릭 드래그 회전)를 재사용하고, 이동은 카메라 기준, 몸은 이동 방향으로 회전한다. `MonsterFirstPersonCamera.cs`는 삭제됐다. 아래 §6.2의 "1인칭 확정" 서술은 이 결정으로 대체된다.
> - **"쿠키만 GameScene 이동, 괴물은 GameLobbyScene 대기"(§2.3/§2.4)를 구현했다.** 아래 머리말의 "구현하지 않았다"는 서술은 더 이상 유효하지 않다. 괴물 클라이언트만 `AutomaticallySyncScene`을 끄고, 시작 신호(`PaintPhaseEndTime`) 수신 시 메시지 큐를 멈춘 채 대기실에서 카운트다운한 뒤 혼자 `GameScene`을 로드한다(`MonsterLobbyWaitController`). 방장이 괴물이 되면 방장 권한을 쿠키에게 넘긴다(`MasterHandoffGuard`). 시작 버튼은 괴물이 확정돼야 활성화된다.
